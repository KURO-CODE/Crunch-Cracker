...
REPO="git clone https://github.com/KURO-CODE/Crunch-Cracker.git"
DUMP="Wordlist"
TMP2="KEY"
...
function check_root_perm() {
	...
	if [ "$user" = "root" ]; then
		...
		echo -e "\n${W} [${R}X${W}]${GR}...${R}Y${W}ou are not ${R}Root${W}!\n\n ${G}U${W}se:${Y} sudo ./crunchcracker.sh${E}C"
 		sleep 1
...
function LST {
	...
	if [ $LX = 0 ]; then
		...
		case $min in
			[0-9]*) LX=$(($LX+1));;
			*) echo -e "${W} [${R} $error${W} ]"; sleep 3; LX=0; LST;;
		esac
	fi
	if [ $LX = 1 ]; then
		clear
		...
		case $max in
			[0-9]*) LX=$(($LX+1));;
			*) echo -e "${W} [${R} $error${W} ]"; sleep 3; LX=1; LST;;
		esac
	fi
	if [ $LX = 2 ]; then	
		clear
...
function Check_List {
	...
	if [ ! -f "$DUMP/$WORDLIST" ]; then
		clear
		LOGO
		echo -e "${W}[${R}x${W}]${GR}...${W}file do Not exist!";
		sleep 3
		RLIST
	else
		echo -e " ";
...
function cap_trap {
	case $Place in
		...
		"PIPELINE") KILLPROC; NETWORKRESTOR; EXITMENU;;
		"END_CRK") KILLPROC; NETWORKRESTOR; EXITMENU;;
